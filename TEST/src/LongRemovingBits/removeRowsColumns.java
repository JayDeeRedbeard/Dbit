			...
			if (removingBits.numberOfnotvalidColumns(tmp) > 64) {
				while (!finish) {
					...
					if (!finish) {
						...
						tem=readdata.make1DatafileLong.numberOfTruesInColumn.get(y).get(e);
						readdata.make1DatafileLong.numberOfTruesInColumn.get(d).set(c,tem);
						
						System.out.println("tem= "+ tem);
						System.out.println("temdc= "+ readdata.make1DatafileLong.numberOfTruesInColumn.get(d).get(c));
						System.out.println("temdc= "+  (stuff.DirtyLittleHelpers.getBitAtPosition(readdata.longData.validColumn.get(d), c)));
						
						longData.validColumn.set(d, stuff.DirtyLittleHelpers.setBitAtPosition(readdata.longData.validColumn.get(d), c, true));
						System.out.println("column d.c= "+ (stuff.DirtyLittleHelpers.getBitAtPosition(readdata.longData.validColumn.get(d), c)));
						
						readdata.make1DatafileLong.numberOfTruesInColumn.get(y).set(e, -1);
						longData.validColumn.set(y,stuff.DirtyLittleHelpers.setBitAtPosition(longData.validColumn.get(y), e, false));
						d_temp = d;
						y_temp = y;
						breakcounter = false;
						counter++;
				...
				while (readdata.longData.validColumn.get(readdata.longData.validColumn.size() - 1) == (0L)
					...
					for (int row = 0; row < tmp.size(); row++) {
						if(tmp.get(row).getValid()){
							...
							tmp1.remove(readdata.longData.validColumn.size() - 1);
							DB = new DBit(tmp.get(row).getValue(), true, tmp1);
							tmp.set(row, DB);
						}
					}
