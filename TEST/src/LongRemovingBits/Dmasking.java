						...
						if (tmp.get(k).getValid() && row!=k) {
							for (int d = 0; d < tmp.get(0).getList().size(); d++) {
								...
								if (readdata.make1DatafileLong.numberOfTruesInRow
									...
									if (var == (tmp.get(k).getList().get(d) & longData.validColumn.get(d)) ) { // Auf// row// dominierend																	
										counttrueA++;
//										if (var==0){
//											dominationcounterA++;
//										}
								...
								if (readdata.make1DatafileLong.numberOfTruesInRow
									...
									if (var == (tmp.get(row).getList().get(d) & longData.validColumn.get(d)) ) { // Auf// k	// dominierend								
										counttrueB++;
//										if (var==0){
//											dominationcounterB++;
//										}
