...
/** An implementation of Edmonds's Maximum Matching algorithm for general graphs.  This algorithm is comparatively old
 * - it dates back to 1965 - and is not particularly efficient compared to modern algorithms (it runs in O(n^2 m), when
 * O(sqrt(n) m) algorithms now exist).  However, Edmonds's algorithm is extremely theoretically elegant and can be
 * implemented easily using standard, off-the-shelf utility classes.
 * <p>
 * The key concept powering Edmonds's algorithm is the notion of an alternating path.  Given a graph and matching
 * (G, M), consider any path P in G.  Some of the edges in P will also be in M, while others will not be.  Let's call an
 * edge in the path that's also in M "solid," while an edge in the path not in M "dashed."  A path is called alternating
 * if the edges alternate between solid and dashed.  For example, this path is alternating:
 * <pre>
 *               * ======== * = = = = = * ========= * = = = = = *
 * </pre>
 * As is this one:
 ...
 *                            * = = = = = * ========= *
 * </pre>
 * Note that this also means that a single edge is automatically an alternating path.
 * <p>
 * The main theorem on which Edmonds's matching algorithm is based is the following:
 * <p>
 * "Given a matching (G, M), M is a maximum matching iff G contains no alternating paths between exposed vertices."
 * <p>
 * There are several good proofs of this claim.  One direction is easy - if the graph contains an alternating path
 * between exposed vertices, then we can construct a larger matching by replacing the matched edges in the path with the
 * unmatched edges.  That is, we change
 * <pre>
 *          * = = = = * ======= * = = = = * ======= * = = = = *
 * </pre>
 * into
 ...
 *          * ======= * = = = = * ======= * = = = = * ======= *
 * </pre>
 * This matching has edge more than the previous matching, so M isn't maximum. The inverse direction is a bit trickier;
 * it works by considering the set symmetric difference between a maximum matching and any other matching, then looking
 * at the connected components of this graph; one can be shown to be the necessary alternating path.
 * <p>
 * A collection of alternating paths can be formed together to create an "alternating tree."  An alternating tree is a
 * tree defined with respect to a matching (G, M) as follows:
 * <ul>
 *  <li> (1) The nodes are partitioned into two disjoint groups - "outer vertices" and "inner vertices."
 *  <li> (2) Each inner vertex has degree two, and exactly one of its incident edges is in M.
 *  <li> (3) Each edge connects exactly one inner node and exactly one outer node.
 *  <li> (4) There is a unique outer node that is the root of the tree.
 *  <li> (5) Every path from a vertex to the root is an alternating path.
 *  <li> (6) Every path from an outer vertex to the root begins with an edge in M.
 * </ul>
 * This is a lot of structure to take in at once, so a more informal presentation is probably in order.  You can think
 * of an alternating tree as a tree formed as the concatenation of several "gadgets" formed of a solid edge (an edge in
 * M) joined to a dashed edge (an edge not in M). The alternating tree is then a collection of paths formed from these
 * gadgets concatenated together in such a way that the endpoint of each gadget is either a designated root node or the
 * start point of some other gadget.  The "outer vertices" from (1) are then the start nodes of the gadgets, while the
 * "inner vertices" from (1) are the inner nodes.  This automatically guarantees (2) and (3).  The special root node is
 * the unique outer node mentioned in (4), and the construction easily guarantees (5) and (6).
 * <p>
 * The reason that these "alternating trees" are so useful is that they give a great way to locate alternating paths
 * between exposed vertices.  Suppose that we root an alternating tree at an exposed vertex and then find an edge from
 * an outer node to an exposed vertex.  This means that we then have an alternating path from that exposed vertex to the
 * root of the tree, formed by taking the edge from the exposed vertex to the outer node and the path from the outer
 * node to the root of the tree (which is alternating and starts with a matched edge).  The algorithm thus works by
 * growing this alternating tree from an arbitrary exposed node outward until either every node is part of the tree (in
 * which case no alternating path exists and we have a maximum matching), or until an alternating path is found.
 * <p>
 * There is one last detail to worry about, and that's what happens if there is an unmatched edge in the graph that
 * links together two from two outer nodes of the tree.  If this happens, this means that there is an odd cycle in the
 * graph, and it might be possible for an alternating path to exist in the graph that wouldn't be noticed by the tree
 * (you should draw out a picture here to convince yourself that this is true).  When this happens, the algorithm
 * "contracts" the odd cycle down to a single node, then recursively searches this new graph for an alternating path. If
 * one exists and doesn't pass through this new pseudonode, then it's an alternating path in the old graph.  If it does
 * pass through the new pseudonode, then the path can be expanded back into a path in the original graph by modifying
 * the matching so that the path through the cycle back to the root is alternating (this doesn't affect the cardinality
 * of the matching), then returning the proper alternating path.
 *
 * The algorithm can make at most O(n) iterations of finding alternating paths, since a maximum matching has cardinality
 * at most n/2.  During each iteration of the tree-growing step, we need to consider each edge at most once, since it
 * either goes in the tree, finds an augmenting path, or creates a cycle. Whenever a cycle is found, we can create the
 * graph formed by contracting that cycle in O(m) by scanning over the edges.  Finally, we can have at most O(n)
 * contractions, because each contraction removes at least one node.  This gives the algorithm a runtime of O(n^2 m).
 * 
