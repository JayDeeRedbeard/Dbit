...
/** A class representing an undirected graph where each edge has an associated real-valued length.  Internally, the
 * class is represented by an adjacency list where each edges appears twice - once in the forward direction and once in
 * the reverse.  In fact, this implementation was formed by taking a standard adjacency list and then duplicating the
 * logic to ensure each edge appears twice.
 * 
