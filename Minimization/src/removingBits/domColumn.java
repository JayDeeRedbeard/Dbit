...
public class domColumn {
	...
	public static ArrayList<ArrayList<Boolean>> removeEqualColumns(ArrayList<ArrayList<Boolean>> tmp){
		...
		/** Loeschen von gleichen Spalten aus der ueberdeckungstabelle
		...
		*/
		...
		tmp1.add(tmp2);tmp1.add(tmp3);											//ob sich im Loesungsarray tmp1 schon die gesuchte Spalte befindet
		int counter=0;															//k und z sind die Laufindizes ,um jede Spalte durchzugehen						
			for (int k=0; k<tmp.get(0).size(); k++){							//j - Reihe
				for(int z=0; z<tmp.get(0).size(); z++){				
					for(int j =0;j<tmp.size(); j++){						
						...
						//System.out.println("Zaehler "+counter);														//Zum Testen
					}
					//System.out.println("Zaehler "+counter + " =? "+ tmp.size());										//Zum Testen
					if(counter==tmp.size()){									//Es muessen alle Zeilen in einer Spalten uebereinstimmen
							tmp1.get(0).add(z);
							tmp1.get(1).add(k);
					}
					counter=0;	
			...
			for(int x=tmp1.get(0).size()-1; x>=0;x--){							//Loeschen von Spalte 1 (tmp1)
	...
	public static ArrayList<ArrayList<Boolean>> removetrueColumns(ArrayList<ArrayList<Boolean>> tmp){
		//Diese funktion ist nicht mehr brauchbar da sie von Remove all dominating Columns dominiert wird
		/** Loeschen von Spalten die nur true haben, aus der ueberdeckungstabelle
		@author Jan Dennis Reimer		
		@version1.0
		@param ArrayList<ArrayList<ArrayList<Boolean>>> tmp		Bekommt die 3D-ArrayList uebergeben	(ueberdeckungstabelle)
		@return													3D-ArrayList ohne gleiche Spalten
	...
	public static ArrayList<ArrayList<Boolean>> removeNotDominatingColumns(ArrayList<ArrayList<Boolean>> tmp) throws IOException{
		...
		*/
		...
		for (int x=tmp.get(0).size()-1; x>=0;x--){
			...
			// Sortieralgorithmus Bubble Sort start:  //Hier eig nicht noetig!!!
			int temp1, temp2;
